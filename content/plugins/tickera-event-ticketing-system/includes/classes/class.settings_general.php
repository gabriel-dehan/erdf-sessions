		...
		function get_settings_general_fields() {
			...
			if ( !defined( 'TC_LCK' ) && !defined( 'TC_NU' ) ) {
				$license_settings_default_fields = array(
					array(
						...
						'section'		 => 'license',
						'tooltip'		 => __( 'License Key is required if you want to update plugin from within the WordPress. You can obtain the key from you account page.', 'inv' ),
					),
				);
			}
			...
			$store_settings_default_fields = array(
				...
				array(
					...
					'function'		 => 'tc_yes_no',
					'default_value'	 => 'yes',
					'tooltip'		 => __( 'Show Ticket Owner fields on the Cart page. If this option is not selected, owner info fields will not be collected and shown on the ticket.', 'tc' ),
					'section'		 => 'store_settings'
				),
				array(
